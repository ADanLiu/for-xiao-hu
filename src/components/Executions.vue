...
<script>
  ...
  export default {
    ...
    computed: {
      percentage () {
        const hashL = this.progressBarText.split('').length - this.progressBarText.split('').filter(c => c==='-').length
        ...
        return new Promise((resolve) => {
          ...
          let step = (timestamp) => {
              ...
              if (progressingCount%3 === 0) {
                this.progressBarText = this.progressBarText.replace('-', '❤️')
              }
              progressingCount++
              if (timeGap < duration) {
                ...
                resolve()
                this.progressBarText = this.progressBarText.replace(/-/g, '❤️')
